...
restoretosit=$(mongorestore --verbose --host $MONGO_CLUSTER -d $MONGO_DB_NAME  --nsInclude 'opsserver-main.*' --nsExclude 'opsserver-main.app' --nsExclude 'opsserver-main.apikeys' --nsExclude 'opsserver-main.deliveries' --nsFrom='opsserver-main.*' --nsTo='sit-opsserver.*' --drop --maintainInsertionOrder --gzip --archive=$BACKUPFILE)
